...
{   
    ...
    [SerializeField] private GameObject[] avalibleSlots;
    ...
    //Arrays used to show that last state of the board.
    [SerializeField] private GameObject[] playerLastStates;
...
//---------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    void Update()
    {
        ...
        if (focused == true && (defaultCursor == true || sacrificing == true))
        {
            ...
            //As the player is focused, show them all of the slots that can play a card on
            if (player1playing == true)
            {
                for (int index = 0; index  < player1Feild.Length; index ++)
                {
                    if (player1Feild[index] == null)
                    {
                        avalibleSlots[index].GetComponent<AvalibleSlot>().ShowSlot();
                    }
                }
            }
            ...
            {
                for (int index = 0; index  < player2Feild.Length; index ++)
                {
    ...
    public void NewTurn()
    {
        ...
        if (turnTotal < 5)
        {
            coinsGained = Random.Range(4,6);
        }

        else if (turnTotal < 7)
        {
            coinsGained = Random.Range(5,7);
        }

        else if (turnTotal < 10)
        {
            coinsGained = Random.Range(6,8);
        }

        else
        {
            coinsGained = Random.Range(8,12);
        }
    ...
    public void PlayCard(GameObject slot, int chosenIndex)
    {
        ...
        if (player1playing == true) //Player 1 is currently playing 
        {
            if (player1Feild[index] == null)
            {
                ...
                {
                    infoText.text = "You don't have enough to play that card!";
                }
            }
        }
    ...
    {
        //Scrape all of the sprite of the cards that survived the attack to let the player see what the oponents cards are
        for (int slot = 0; slot < 5; slot++)
        {
            playerLastStates[slot].GetComponent<AttackSlot>().ErasePlayerLasts(); //Clean up all the sprites before adding new ones
            
            //--------------------------------------------------------

            //Setting up the clones for the next turn
            if (player1Feild[slot] != null)
            {
                playerLastStates[slot].GetComponent<AttackSlot>().SetPlayer1State(player1Feild[slot].GetComponent<SpriteRenderer>().sprite); 
            }
            
            if (player2Feild[slot] != null)
            {
                playerLastStates[slot].GetComponent<AttackSlot>().SetPlayer2State(player2Feild[slot].GetComponent<SpriteRenderer>().sprite); 
            } 
        } 
        ...
        Debug.Log("Player2recentShown");

        //---------Load the last seen state of player 2--------------
        for (int i = 0; i < playerLastStates.Length; i++)
        {
            playerLastStates[i].GetComponent<AttackSlot>().ShowPlayer2State();
        }
    ...
    public void DeloadPlayer1()
    {
        ...
        //---------Deload the last seen state of player 2--------------
        for (int i = 0; i < playerLastStates.Length; i++)
        {
            playerLastStates[i].GetComponent<AttackSlot>().TurnInvisible();
        }
    ...
    public void LoadPlayer2()
    {
        ...
        //---------Load the last seen state of player 1--------------
        for (int i = 0; i < playerLastStates.Length; i++)
        {
            playerLastStates[i].GetComponent<AttackSlot>().ShowPlayer1State();
        }
    ...
    public void DeloadPlayer2()
    {
        ...
        //---------Deloadoad the last seen state of player 1--------------
        for (int i = 0; i < playerLastStates.Length; i++)
        {
            playerLastStates[i].GetComponent<AttackSlot>().TurnInvisible();
        }

        gameBoardSprite.flipY = false; //Setting the board back to its normal orientation
    }
    ...
    public void DeloadAll()
    {
        ...
        //Set all the 'Last seen cards' To invisble
        for (int i = 0; i < playerLastStates.Length; i++)
        {
            playerLastStates[i].GetComponent<AttackSlot>().TurnInvisible();
        }
    ...
    public void Defocus()
    {
        ...
        //Getting rid of all the card slot highlighters
        for (int slot = 0; slot < avalibleSlots.Length; slot++)
        {
            avalibleSlots[slot].GetComponent<AvalibleSlot>().HideSlot();
        }
    }
